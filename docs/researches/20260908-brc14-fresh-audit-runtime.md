...
A completed-group audit cannot bootstrap active admission: it requires published adoption and per-Task cleanup, while active currently refuses unavailable exact-revision authority. The new `campaign observe-revision --campaign-id <id>` entry runs only in the initial authorized/preparing state, before any provider operation, and does not require or manufacture an issue-batch intent. Its immutable request binds the exact campaign grant, repository, target revision, profile and sent prompt.
