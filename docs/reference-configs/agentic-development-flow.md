...
9. Use `repo-harness run refresh-current-status` for an explicit `tasks/current.md` preview or `--write` snapshot. The snapshot is an ignored local read model regenerated per worktree, and it never replaces source artifacts.
10. After substantive changes, run project checks and record evidence in `tasks/`. For contract worktrees, `verify-sprint --prepare-acceptance` first recomputes the policy-bound Change Assessment packet; reviewers read its selected paths, closed reasons, and required oracles before external acceptance. Hook reports remain advisory. Then run Waza `/check`, start host-aware external acceptance in parallel, fill the review artifact from both verdicts, and use `repo-harness-ship` for default PR closeout. It calls `repo-harness run contract-worktree finish --no-merge`, pushes the `codex/<slug>` branch, and opens a draft PR. Use `repo-harness-ship --local-merge` only when an explicit maintainer workflow wants the local single-publication path: checkpoint and lifecycle commits remain on the source branch, while the target receives one tree-equivalent work-package commit.
