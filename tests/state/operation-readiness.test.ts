...
describe('evaluateReadiness fixture-driven matrix', () => {
  test('architecture projection policy keeps provider and apply orthogonal and fail-closed', () => {
    expect(readArchitectureProjectionPolicy({ architecture: {
      ...
      projection_version: '0.5.9',
      projection_timeout_ms: 120000,
    } })).toEqual({ provider: 'archctx', applyMode: 'manual', failureGate: 'strict', requiredVersion: '0.5.9', timeoutMs: 120000 });
    expect(readArchitectureProjectionPolicy({ architecture: {
    ...
    } })).toEqual({ provider: 'disabled', applyMode: 'disabled', failureGate: 'advisory', requiredVersion: '0.5.9', timeoutMs: 120000 });
    expect(() => readArchitectureProjectionPolicy({ architecture: {
      projection_provider: 'disabled',
    ...
    } })).toEqual({ provider: 'archctx', applyMode: 'automatic', failureGate: 'advisory', requiredVersion: '0.5.9', timeoutMs: 300000 });
    expect(() => readArchitectureProjectionPolicy({ architecture: {
      projection_provider: 'archctx',
      projection_apply: 'automatic',
      projection_version: '0.5.9',
