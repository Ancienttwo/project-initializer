...
describe('Stop-time restamp auto-publication', () => {
  ...
  test('leaves the strict projection failure gate criteria untouched', () => {
    const strictPolicy = { architecture: { projection_provider: 'archctx', projection_apply: 'automatic', projection_version: '0.5.8', projection_failure_gate: 'strict' } };
  ...
  test('converges: the published restamp keeps the next drain idle without a provider run', () => {
    const root = fixture({ architecture: { projection_provider: 'archctx', projection_apply: 'automatic', projection_version: '0.5.8' } });
    seedReceipt(root, RESTAMP);
