...
test('campaign exposes closeout while step retains local-host planning handoff', () => {
  ...
  expect(command.commands.map(c => c.name())).toEqual(['close-not-planned', 'closeout', 'start', 'transition', 'status', 'observe-revision', 'audit', 'author', 'author-followup', 'step', 'adopt']);
