...
function policy() { return readRefactorPolicy({ refactor: { mode: "off", provider: "archctx", stages: { scan: { provider_version: "0.5.8", required_features: ["module-statistics-v1", "refactor-assessment-v1", "recommendation-v3"] }, verify: { provider_version: "0.5.8", required_features: ["refactor-resolution-v1"] } }, require_cutover_closure: false, require_post_merge_measurement: false } }); }
...
describe("refactor archctx provider", () => {
  test("handshakes once and validates a scan result bound to the requested identity", () => {
    ...
    const run = (_binary: string, args: readonly string[]) => {
      ...
      const value = args[0] === "capabilities"
        ? { schemaVersion: "archcontext.capabilities/v1", package: { name: "archctx", version: "0.5.8" }, features: ["module-statistics-v1", "refactor-assessment-v1", "recommendation-v3", "refactor-resolution-v1"] }
  ...
  test("rejects stale identity and missing features before returning data", () => {
    const scanRun = (_binary: string, args: readonly string[]) => ({ status: 0, signal: null, stderr: "", stdout: JSON.stringify(args[0] === "capabilities" ? { schemaVersion: "archcontext.capabilities/v1", package: { name: "archctx", version: "0.5.8" }, features: ["module-statistics-v1"] } : {}) });
    ...
    const staleRun = (_binary: string, args: readonly string[]) => {
      ...
      const value = args[0] === "capabilities"
        ? { schemaVersion: "archcontext.capabilities/v1", package: { name: "archctx", version: "0.5.8" }, features: ["module-statistics-v1", "refactor-assessment-v1", "recommendation-v3"] }
  ...
  test("record and verify use their frozen CLI contracts and preserve upstream errors", () => {
    ...
    const run = (_binary: string, args: readonly string[]) => {
      calls.push([...args]);
      let value: unknown;
      if (args[0] === "capabilities") value = { schemaVersion: "archcontext.capabilities/v1", package: { name: "archctx", version: "0.5.8" }, features: ["module-statistics-v1", "refactor-assessment-v1", "recommendation-v3", "refactor-resolution-v1"] };
    ...
    const upstreamError = (_binary: string, args: readonly string[]) => ({ status: args[0] === "capabilities" ? 0 : 1, signal: null, stderr: "", stdout: JSON.stringify(args[0] === "capabilities" ? { schemaVersion: "archcontext.capabilities/v1", package: { name: "archctx", version: "0.5.8" }, features: ["module-statistics-v1", "refactor-assessment-v1", "recommendation-v3", "refactor-resolution-v1"] } : { schemaVersion: "archcontext.envelope/v1", ok: false, requestId: "refactor.scan", error: { code: "AC_REFACTOR_STALE", message: "stale" } }) });
