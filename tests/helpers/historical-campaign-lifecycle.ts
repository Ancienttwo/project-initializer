...
export async function prepareHistoricalCodexInvocation(input: Parameters<typeof import('../../src/effects/automation/campaign-runtime').prepareCampaignCodexInvocation>[0]): Promise<CampaignCodexInvocation> {
  ...
  const make = (probe: boolean): CampaignContainer => {
    const directory = join(common, 'repo-harness/campaign-containers', randomUUID()); mkdirSync(directory, { recursive: true });
...
function modeledContainerTerminal(handle: CampaignContainer, stdout: string, stderr: string, observation: { exit_code: number | null; output_complete?: boolean; termination_cause?: string; started?: boolean }): string {
