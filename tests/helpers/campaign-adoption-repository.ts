...
export async function createAdoptionRepository(mode: 'shadow' | 'active' = 'active', rounds = 1, capability = CAP, metadata: Record<string, unknown> = {}, files: Record<string, string> = {}, limits: { max_agent_turns?: number; max_runner_invocations?: number; max_parallel_tasks?: 1 | 2 | 3; group_count?: 1 | 2 | 3; max_provider_calls?: number; max_successful_acquisitions?: number; liveness_policy?: LeaseLivenessPolicyV1 } = {}) {
  ...
  const authorization = sealProgramAuthorization({ authorization_id: 'auth-1', repository_id: repository, target_ref: 'refs/heads/main', target_revision: revision, work_graph_revision: 'a'.repeat(64), allowed_work_package_ids: ['campaign-1'], allowed_risk_tiers: ['low'], merge_mode: 'manual', allowed_merge_method: 'squash', max_repair_cycles: 2, budget: { max_agent_turns: limits.max_agent_turns ?? 10, max_successful_acquisitions: limits.max_successful_acquisitions ?? 2, max_runner_invocations: limits.max_runner_invocations ?? 10, max_provider_failures: 2, max_consecutive_no_progress_steps: 2, max_repair_cycles: 2, max_wall_clock_seconds: 3600, max_input_tokens: null, max_output_tokens: null, max_cost_micros: null }, contract_scope: 'contract_less', contract_path: null, campaign: { ...(limits.liveness_policy ? { liveness_policy: limits.liveness_policy } : {}), campaign_id: 'campaign-1', group_count: limits.group_count ?? 1, issues_per_group: 2, allowed_issue_kinds: ['bugfix', 'test_gap'], max_parallel_tasks: limits.max_parallel_tasks ?? 2, transient_retry: { max_consecutive_failures: 3, initial_backoff_ms: 1, maximum_backoff_ms: 4 }, issue_author: 'gpt_pro', local_parent_host: 'codex', chrome_profile_directory: 'Profile 1', max_authoring_rounds_per_group: rounds, max_controller_steps: 100, max_provider_calls: limits.max_provider_calls ?? 100, require_fresh_main_audit: true }, issued_by: 'owner', issued_at: AT, expires_at: '2027-09-05T00:00:00.000Z' });
