...
const intent = { campaign_id: 'campaign', group_number: 1, provider_repository: 'acme/widgets', repository_id: 'repo', target_ref: 'refs/heads/main', base_main_sha: 'a'.repeat(40), allowed_issue_kinds: ['bugfix', 'test_gap'] } as unknown as Parameters<typeof buildIssueAuthoringPrompt>[0];
for (const operation of ['initial', 'fill_missing', 'edit_issue'] as const) test(`${operation} renders the complete metadata contract and a parser-valid example`, () => {
  const prompt = buildIssueAuthoringPrompt(intent, operation, ['01'], '123', 'https://github.com/acme/widgets/issues/1');
