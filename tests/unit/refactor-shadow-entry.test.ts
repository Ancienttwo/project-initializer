...
function fixture(extraFiles: string[] = []) {
  ...
  const provider = { consumerRoot: process.cwd(), run: (_binary: string, args: readonly string[]) => {
    let value: unknown;
    if (args[0] === 'capabilities') value = { schemaVersion: 'archcontext.capabilities/v1', package: { name: 'archctx', version: '0.5.8' }, features: ['module-statistics-v1', 'refactor-assessment-v1', 'recommendation-v3'] };
