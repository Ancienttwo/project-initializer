...
describe("refactor provider contract", () => {
  test("accepts only the exact stage version and feature set", () => {
    ...
    expect(() => assertRefactorCapabilities({ schemaVersion: "archcontext.capabilities/v1", package: { name: "archctx", version: "0.5.8" }, features: [...REFACTOR_SCAN_FEATURES] }, stage)).not.toThrow();
    for (const value of [
      ...
      { schemaVersion: "archcontext.capabilities/v1", package: { name: "archctx", version: "0.5.8" }, features: ["module-statistics-v1"] },
    ]) {
