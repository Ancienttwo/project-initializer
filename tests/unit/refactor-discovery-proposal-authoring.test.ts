  ...
  test('pins the first scan identity before asking the provider to assess a proposal', () => {
    ...
    try {
      assessRefactorProposal({ discovery, candidateAlias: 'C01', proposal }, process.cwd(), { consumerRoot: process.cwd(), run: (_binary, args) => {
        if (args[0] === 'capabilities') return { status: 0, signal: null, stderr: '', stdout: JSON.stringify({ schemaVersion: 'archcontext.capabilities/v1', package: { name: 'archctx', version: '0.5.8' }, features: ['module-statistics-v1', 'refactor-assessment-v1', 'recommendation-v3'] }) };
