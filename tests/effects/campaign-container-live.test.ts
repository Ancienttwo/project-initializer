...
import { prepareCampaignContainer, runCampaignContainer, reconcileCampaignContainer, reconcileCampaignContainerPreparation, campaignContainerDirectory, type CampaignContainer } from '../../src/effects/automation/campaign-container';
...
afterEach(() => {
  for (const item of owned.splice(0)) {
    ...
    if (item.handle) rmSync(item.handle.directory, { recursive: true, force: true });
...
test.skipIf(!image)('protected control journal is absent from every workload mount and alias', async () => {
  const f = await prepare(['/usr/local/bin/node', '-e', "console.log('no-model')"]);
  const request = JSON.parse(readFileSync(join(f.handle.directory, 'request.json'), 'utf8'));
  for (const mount of request.expected.mounts) {
    expect(f.handle.directory === mount.Source || f.handle.directory.startsWith(mount.Source + '/')).toBe(false);
  }
}, 20000);

for (const phase of ['configuration', 'start', 'terminal']) test.skipIf(!image)(`Docker controller death at ${phase} publication reconciles known identity without another start`, async () => {
  const root = mkdtempSync(join(tmpdir(), 'brc-preparation-crash-'));
  mkdirSync(join(root, 'work')); mkdirSync(join(root, 'common'));
  ...
  try {
    ...
    if (request) expect(Bun.spawnSync(['docker', '--host', request.endpoint, 'rm', '-f', request.name], { timeout: 5000 }).exitCode).toBe(0);
