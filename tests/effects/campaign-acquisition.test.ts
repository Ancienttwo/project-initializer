...
async function readyFixture(twoEngineers = false) {
  ...
  for (const directory of ['agents/engineers/profiles', 'agents/engineers/sops', '.ai/harness/sprint', 'tasks/contracts', 'tasks/evidence']) mkdirSync(join(f.root, directory), { recursive: true });
  ...
  for (let index = 0; index < 2; index++) {
    ...
    writeFileSync(join(f.root, guard), 'test guard'); writeFileSync(join(f.root, evidence), `${guard}\nPRE_FIX_EXIT=1\n`);
