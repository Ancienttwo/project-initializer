...
const { runChild: packagedRunChild } = await import(new URL('../../assets/templates/helpers/contract-run.ts', import.meta.url).href) as { runChild: typeof runChild };
...
afterEach(() => {
  for (const root of roots.splice(0)) {
    const journals = campaignContainerJournalRoot(join(root, '.git'));
...
if(process.argv.at(-1)==='worker_nonzero')process.exit(7);
const text=process.argv.at(-1)==='verifier_fail'?JSON.stringify({verdict:'fail',review:'model-free rejection'}):JSON.stringify({writable});
...
`);
  ...
  for (const [surface, childRunner] of [['source', runChild], ['packaged', packagedRunChild]] as const) {
    test(`${surface} actual consumer rejects simultaneous log and summary replacement at the consumption barrier`, async () => {
      const f = fixture('worker'); const invocation = await prepareCampaignCodexInvocation(f.input);
      const child = await childRunner('worker', 'codex-exec:worker', f.root, f.root, { REPO_HARNESS_PACKAGE_ROOT: join(import.meta.dir, '../..') }, f.input.deadline_ms, undefined, invocation);
      const original = observeCampaignCodexTerminal({ ...child, invocation, worktree: f.root });
      expect(original.state).toBe('terminal');
      // Explicit barrier: managed attach has exited and protected publication completed;
      // the parent consumer has not yet consumed these attacker-controlled copies.
      let release!: () => void;
      const consumptionBarrier = new Promise<void>(resolve => { release = resolve; });
      const forged = readFileSync(join(f.root, child.stdout_path), 'utf8').split('\n').filter(line => !line.includes('command_execution')).join('\n');
      writeFileSync(join(f.root, child.stdout_path), forged); writeFileSync(join(f.root, child.stderr_path), '');
      const replacement = { ...child, output_sha256: { stdout: sha(forged), stderr: sha('') }, output_complete: true };
      writeFileSync(join(f.root, 'worker.bounded-result.json'), JSON.stringify(replacement));
      release(); await consumptionBarrier;
      const observed = observeCampaignCodexTerminal({ ...replacement, invocation, worktree: f.root });
      expect(observed.state).toBe('unknown'); expect(observed.supervision_proven).toBe(false); expect(observed.runtime_effect_inactive).toBeNull();
    }, 30000);
    for (const scenario of ['worker_nonzero', 'verifier_fail']) test(`${surface} real Docker child reaches actual finish(fail) and idempotent replay: ${scenario}`, async () => {
      const result = Bun.spawn([process.execPath, join(import.meta.dir, '../fixtures/brc-audit/finish-failure.ts'), scenario], {
        env: { ...process.env, BRC_TEST_RUNTIME_IMAGE: modelFreeImage, BRC_TEST_HELPER: surface }, stdout: 'pipe', stderr: 'pipe',
      });
      const timer = setTimeout(() => result.kill('SIGKILL'), 55000);
      try {
        const output = Promise.all([new Response(result.stdout).text(), new Response(result.stderr).text()]);
        const code = await result.exited; const [stdout, stderr] = await output;
        expect(code, stdout + stderr).toBe(0); expect(stdout).toContain('actual finish(fail) settled and replayed');
      } finally { clearTimeout(timer); result.kill('SIGKILL'); await result.exited; }
    }, 60000);
  }
  test('actual consumer rejects recomputed invocation identity and container substitutions', async () => {
    const f = fixture('worker'); const invocation = await prepareCampaignCodexInvocation(f.input);
    ...
    for (const [field, value] of [['dispatch_id', sha('other')], ['claim_id', 'other'], ['lease_generation', 2], ['binding_generation', 2]] as const) {
      const { invocation_sha256, ...body } = invocation;
      const changed = { ...body, identity: { ...body.identity, [field]: value } };
      const forged = { ...changed, invocation_sha256: canonicalMessageDigest(changed) };
      expect(observeCampaignCodexTerminal({ ...child, invocation: forged, worktree: f.root }).runtime_effect_inactive).toBeNull();
    }
    const { invocation_sha256, ...body } = invocation;
    const changed = { ...body, container: { ...body.container, container_id: 'f'.repeat(64) } };
    expect(observeCampaignCodexTerminal({ ...child, invocation: { ...changed, invocation_sha256: canonicalMessageDigest(changed) }, worktree: f.root }).runtime_effect_inactive).toBeNull();
    for (const delta of [{exit_code: 1}, {output_complete: false}, {container_receipt_sha256: sha('other')}]) {
      expect(observeCampaignCodexTerminal({ ...child, ...delta, invocation, worktree: f.root }).runtime_effect_inactive).toBeNull();
    }
  }, 30000);
