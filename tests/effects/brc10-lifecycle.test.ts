...
import { readdirSync, existsSync, rmSync, writeFileSync, readFileSync, mkdirSync, chmodSync } from 'fs';
...
test.skipIf(!process.env.BRC_TEST_CONTAINER_IMAGE)('Docker prepareChild death before invocation publication reconciles without launch or charge', async () => {
  const { reconcileAndRecoverCampaignDispatch } = await import('../../src/effects/automation/campaign-recovery');
  ...
  const code = `import { mock } from 'bun:test';
    ...
    const runtime=await import(${JSON.stringify(join(import.meta.dir, '../../src/effects/automation/campaign-runtime'))});
    mock.module(${JSON.stringify(join(import.meta.dir, '../../src/effects/automation/campaign-runtime'))},()=>({...runtime,async prepareCampaignCodexInvocation(input){await runtime.prepareCampaignCodexInvocation(input);process.kill(process.pid,'SIGKILL')}}));
  ...
  const journals = campaignContainerJournalRoot(join(f.root, '.git'));
  ...
  try {
    ...
    if (existsSync(journals)) for (const directory of readdirSync(journals)) {
      const request = JSON.parse(readFileSync(join(journals, directory, 'request.json'), 'utf8'));
