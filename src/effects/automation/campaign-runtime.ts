...
import { prepareCampaignContainer, runCampaignContainer, assertCampaignContainerBinding, readCampaignContainerReceipt, reconcileCampaignContainer, readCampaignContainerInterruption, reconcileCampaignContainerPreparation, campaignContainerDirectory } from './campaign-container';
...
export function observeCampaignCodexPreparationInterruption(preparation: CampaignCodexPreparation, worktree: string) {
  ...
  for (const identity of [{ ...preparation.identity, phase: 'version' }, preparation.identity]) {
    const directory = campaignContainerDirectory(common, identity);
    if (!existsSync(directory)) continue;
    const handle = JSON.parse(readFileSync(resolve(directory, 'created.json'), 'utf8'));
    if (handle.directory !== directory) throw new Error('container preparation handle differs');
    const request = JSON.parse(readFileSync(resolve(directory, 'request.json'), 'utf8'));
    const { request_sha256, ...body } = request;
    if (request_sha256 !== handle.request_sha256 || canonicalMessageDigest(body) !== request_sha256
      || request.deadline_ms !== preparation.deadline_ms || canonicalMessageDigest({ identity: request.identity }) !== canonicalMessageDigest({ identity })) throw new Error('container preparation binding differs');
    proofs.push(readCampaignContainerInterruption(handle));
  }
