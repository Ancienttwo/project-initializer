...
export async function prepareCampaignContainer(input: {
  ...
  const directory = join(common, 'repo-harness', 'campaign-containers', randomUUID());
...
export function assertCampaignContainerBinding(handle: CampaignContainer, input: {
  ...
  const parent = join(realpathSync(input.common), 'repo-harness', 'campaign-containers');
  if (!/^[a-f0-9-]{36}$/.test(basename(handle.directory)) || dirname(handle.directory) !== parent
