...
/** Host-owned storage is outside Git/workspace mounts, including their read-only views. */
export function campaignContainerJournalRoot(common: string): string {
  return join(realpathSync(homedir()), '.repo-harness', 'campaign-containers', canonicalMessageDigest({ common: realpathSync(common) }).slice(7));
}
export function campaignContainerDirectory(common: string, identity: unknown): string {
  return join(campaignContainerJournalRoot(common), canonicalMessageDigest({ identity }).slice(7));
}
function assertUnexposedJournal(directory: string, sources: string[]): void {
  for (const source of sources) {
    const canonical = realpathSync(source);
    if (directory === canonical || directory.startsWith(canonical + '/')) throw new Error('container control storage overlaps a workload mount');
  }
}
...
export async function prepareCampaignContainer(input: {
  ...
  assertUnexposedJournal(directory, [worktree, common]);
  mkdirSync(directory, { recursive: true, mode: 0o700 });
  if (realpathSync(directory) !== directory) throw new Error('container control storage has a filesystem alias');
  ...
  assertUnexposedJournal(directory, [worktree, common, ...(auth ? [auth] : [])]);
  for (const source of [worktree, common]) {
    if (socket === source || socket.startsWith(source + '/')) throw new Error('workload mount exposes the Docker endpoint');
  }
...
export function assertCampaignContainerBinding(handle: CampaignContainer, input: {
  ...
  const parent = campaignContainerJournalRoot(input.common);
  if (!/^[a-f0-9]{64}$/.test(basename(handle.directory)) || dirname(handle.directory) !== parent
    || realpathSync(handle.directory) !== handle.directory) throw new Error('container journal is outside protected host storage');
  ...
  assertUnexposedJournal(handle.directory, mounts.map((m: any) => m.Source));
...
export function validateCampaignContainerReceipt(handle: CampaignContainer, receipt: string, stdout: string, stderr: string): boolean {
  try { readCampaignContainerReceipt(handle, receipt, stdout, stderr); return true; } catch { return false; }
}
...
export async function reconcileCampaignContainerPreparation(input: {
  ...
  assertUnexposedJournal(directory, request.expected.mounts.map((m: any) => m.Source));
