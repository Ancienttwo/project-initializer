...
P3: Use existing runHelper with trustedPackage=true for both mutations, retaining exact arguments, cwd, output bounds and error compensation. No repository-script or source-override fallback. No admission, grant, budget, Docker or Oracle changes. This small import uses the established runtime utility currently located under cli/runtime; moving that module is outside this repair. At larger fleet scale subprocess provisioning remains the existing serialized transaction cost.
