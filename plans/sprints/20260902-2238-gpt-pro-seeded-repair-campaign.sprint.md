...
- BRC9: the #287 closed-outcome/retry-admission and #282 campaign-step/provider-budget prerequisites are published. Their acceptance is limited to those prerequisite packages. Heartbeat consumption is a separate in-progress package; acquisition accounting, writable dispatch/attempt binding, per-task repair accounting, transient retry policy, and adoption terminal ordering remain unaccepted. TaskAutomationAttemptV1 requires real Work Package/Claim/Lease/dispatch identity; the existing delegated-run runtime is read-only. Keep row 11 pending until its complete acceptance boundary is met, and keep BRC10 and later consumers blocked. Partial packages must not use the whole BRC9 row as a completion binding. Evidence and boundaries: `docs/researches/20260905-repair-campaign-sprint-execution-boundaries.md`.
