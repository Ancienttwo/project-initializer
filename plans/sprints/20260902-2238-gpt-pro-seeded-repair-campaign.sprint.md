...
- BRC9: #282/#287 implementations are present, but their current contracts do not expose separate campaign-step, authoring-round and provider-call limits, per-task repair accounting, or a pre-adoption campaign attempt identity. TaskAutomationAttemptV1 requires real Work Package/Claim/Lease/dispatch identity and cannot represent GPT authoring. Keep row 11 pending/blocked until the upstream campaign-required contract subset is available; do not build a second budget authority inside this Sprint. BRC10 and later consumers remain dependent on this gap. Evidence and ownership: `docs/researches/20260905-repair-campaign-sprint-execution-boundaries.md`.
