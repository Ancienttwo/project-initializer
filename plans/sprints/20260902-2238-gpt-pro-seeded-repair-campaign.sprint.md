...
- Connector 探针证明了样本回答命中，未证明 exact revision；BRC6a 是明确未完成的修正项。
...
- BRC6a owns the open readback proof defect. Existing BRC6 completion records retain their historical
  subject and implementation evidence; they do not satisfy the corrected exact-SHA activation boundary.
  No runtime guard was added by this documentation amendment; current code must not be described as fixed.
- BRC9: #287 retry-admission, #282 campaign-step/provider-budget, heartbeat consumption and the writable
  worker/attempt bridge are published as separate packages. Remaining consumers are acquisition accounting,
  per-task repair accounting, transient retry policy and adoption terminal/provider ordering. Keep BRC9
  pending and BRC10/BRC13/BRC14/BRC15 active acceptance blocked until their full prerequisites pass.
